...
class etcd_backup_validator(object):
    ...
    def test_etcd_backup_on_cluster_error(self, uuid, backup_path=None):
        ...
        backup_conf_dict = {'isEtcdBackupEnabled': 1,
                            'intervalInMins': 1,
                            'maxIntervalBackupCount': 2
                            }

        # Validate qbert rejects requests of invalid backup interval
        try:
            # By default qbert expects the interval to be greater than 10 mins
            ...
            log.info("Attempting etcd backup setting with intervalInMins set as 1")
            self.update_cluster(uuid, backup_conf_dict, backup_path)
    ...
    def test_etcd_backup_on_cluster(self, uuid, backup_path=None):
        ...
        edited_conf = {"\"minBackupIntervalMins\": [0-9]\+": "\"minBackupIntervalMins\": 1"}
    ...
    def test_etcd_backup_with_timestamp(self, uuid, backup_path=None):
        if not backup_path:
            backup_path = '/var/opt/pf9/etcd-backup-timestamp'

        cronjob_schedule_time = datetime.now() + timedelta(minutes = 2)
        cronjob_schedule_time = cronjob_schedule_time.strftime("%H:%M")

        backup_conf_dict = {'isEtcdBackupEnabled': 1,
                            'dailyBackupTime': cronjob_schedule_time,
                            'maxTimestampBackupCount': 2,
                            }
    ...
    def update_cluster(self, uuid, etcd_backup_conf, backup_path):
        etcd_storage_dict = {
            'storageType': 'local',
            'storageProperties': {
        ...
        # Add etcd backup storage details
        etcd_backup_conf.update(etcd_storage_dict)

        body = {'etcdBackup': etcd_backup_conf}
...
def test_etcdbackup(qbert, du_fqdn, cluster_uuid):
    ...
    validator.test_etcd_backup_with_timestamp(cluster_uuid)
