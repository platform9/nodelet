...
from pf9lab.lagoon import checkout_hosts
...
def create_hosts(host_provider, num_hosts, tag, template_key, flavor_name, network_name=None):
    hosts = None
    if os.getenv('USE_HOST_POOL', '').lower() in ['true', 'yes', '1']:
        hosts = checkout_hosts(tag, host_provider, 'raw_kube_testbed', template_key, num_hosts)
        if hosts and 'SUITE_RESULTS_DIR' in os.environ:
            suite_result_dir = os.getenv('SUITE_RESULTS_DIR')
            hostpool_status_file = os.path.join(suite_result_dir, 'used_hostpool.txt')
            Path(hostpool_status_file).touch()
