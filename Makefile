...
PF9_KUBE_VERSION := $(shell ${AGENT_SRC_DIR}/get-version.bash)
PF9_KUBE_RELEASE := $(shell ${AGENT_SRC_DIR}/get-release.bash)
...
PF9_KUBE_DEB_FILE_WITH_VERSION := $(AGENT_BUILD_DIR)/nodelet-$(PF9_KUBE_VERSION)-$(PF9_KUBE_RELEASE).x86_64.deb
...
$(COMMON_SRC_ROOT): easyrsa $(AUTHBS_SRC_DIR) bouncer-docker-image kubernetes nodelet cni-plugins containerd runc nerdctl crictl calicoctl etcdctl etcd_raft_checker pf9kube-addr-conv pf9kube-ip_type virtctl jq
...
$(PF9_KUBE_RPM_FILE): | $(RPM_SRC_ROOT)
	...
	rpmbuild -bb \
	    ...
	    --define "_release $(PF9_KUBE_RELEASE)" \
