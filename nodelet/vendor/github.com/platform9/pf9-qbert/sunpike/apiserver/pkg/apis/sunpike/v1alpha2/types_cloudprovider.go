...
const (
	...
	CloudProviderTypeGKE     CloudProviderType = "gke"
...
func (c *CloudProviderStatus) SetCondition(condition Condition) {
	for i := range c.Conditions {
		existing := c.Conditions[i]
		if existing.Type == condition.Type {
			if !existing.EqualsWithoutTransitionTime(condition) {
...
func (c *CloudProviderStatus) GetCondition(conditionType ConditionType) (Condition, bool) {
...
type AzureCloudProviderSpec struct {
	...
	// (3) subscriptionID: the subscription ID of the Azure user account.
