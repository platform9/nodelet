...
func Extract(tgzFile string, destDir string) error {
	...
	for {
		...
		case mode.IsRegular():
			...
			//TODO: if the file/executable we are opening here is already opened or running we get error.
			//e.g. if containerd is running we get error while copying/creating containerd binary to specified loc.
