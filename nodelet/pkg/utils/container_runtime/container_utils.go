...
func (c *ContainerUtility) EnsureContainersDestroyed(ctx context.Context, containers []containerd.Container, timeoutStr string) error {
	...
	for _, container := range containers {
		...
		err = c.StopContainer(ctx, container, timeoutStr)
		if err != nil {
			zap.S().Infof("couldn't stop the container: %s :%s", container.ID(), err)
			return errors.Wrapf(err, "couldn't stop the container: %s", container.ID())
		}
...
func (c *ContainerUtility) StopContainer(ctx context.Context, container containerd.Container, timeoutStr string) error {
	...
	if err != nil {
		zap.S().Info("err from parseduration:%v", err)
		return err
	}
	task, err := container.Task(ctx, cio.Load)
	if err != nil {
	...
	status, err := task.Status(ctx)
	if err != nil {
		zap.S().Info("err from task.status :%s", err)
		return err
	}
	...
	if err != nil {
		zap.S().Info("err from task.wait :%s", err)
		return err
	}
	...
	if timeout > 0 {
		...
		if err := task.Kill(ctx, syscall.SIGTERM); err != nil {
			zap.S().Info("err from task.kill in timeout :%s", err)
			return err
		}
		...
		if ctx.Err() != nil {
			zap.S().Info("err from ctx.err :%s", ctx.Err())
			return ctx.Err()
		}
	}

	if err := task.Kill(ctx, syscall.SIGKILL); err != nil {
		zap.S().Info("err from task.kill :%s", err)
		return err
